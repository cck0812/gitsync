...
def templateBaseParameters = [
  ...
  // user:                     '',
  // volumesFromString:        '',
  mountsString:             'type=bind,src=/etc/docker/daemon.json,dst=/etc/docker/daemon.json\ntype=bind,src=/etc/default/docker,dst=/etc/default/docker',
]
...
// https://github.com/jenkinsci/docker-plugin/blob/master/src/main/java/com/nirima/jenkins/plugins/docker/DockerCloud.java
 def cloudParameters = [
  ...
  serverUrl:                   'tcp://docker:2376',
  name:                        'local-docker-cloud',
  containerCap:                100, // 0 means no cap
  // credentialsId:            '',
  connectTimeout:              60,
  readTimeout:                 60,
  // version:                  '',
